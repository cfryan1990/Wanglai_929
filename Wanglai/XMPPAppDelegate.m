...
#import "WangLaiViewController.h"
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    messageVC.tabBarItem.title = @"新消息";
    messageVC.tabBarItem.image = [UIImage imageNamed:@"tabbar_newmessage.png"];
    UINavigationController * messageNC = [[UINavigationController alloc] initWithRootViewController:messageVC];
    messageNC.navigationBar.barStyle = UIBarStyleBlack;
    messageNC.navigationBar.translucent = NO;
    //    messageVC.tabBarItem.badgeValue = @"3";
    
    WangLaiViewController * wanglaiVC = [[WangLaiViewController alloc] init];
    wanglaiVC.tabBarItem.title = @"往来";
    wanglaiVC.tabBarItem.image = [UIImage imageNamed:@"tabbar_wanglai.png"];
    UINavigationController * wanglaiNC = [[UINavigationController alloc] initWithRootViewController:wanglaiVC];
    wanglaiNC.navigationBar.barStyle = UIBarStyleBlack;
    wanglaiNC.navigationBar.translucent = NO;
    
    
    ContactViewController * contactVC = [[ContactViewController alloc] init];
    contactVC.tabBarItem.title = @"通讯录";
    contactVC.tabBarItem.image = [UIImage imageNamed:@"tabbar_contact.png"];
    UINavigationController * contactNC = [[UINavigationController alloc] initWithRootViewController:contactVC];
    contactNC.navigationBar.barStyle = UIBarStyleBlack;
    contactNC.navigationBar.translucent = NO;
    
    
    MeViewController * meVC = [[MeViewController alloc] init];
    meVC.tabBarItem.title = @"我";
    meVC.tabBarItem.image = [UIImage imageNamed:@"tabbar_me.png"];
    UINavigationController * meNC = [[UINavigationController alloc] initWithRootViewController:meVC];
    meNC.navigationBar.barStyle = UIBarStyleBlack;
    meNC.navigationBar.translucent = NO;
    
    UITabBarController * tabBarController = [[UITabBarController alloc] init];
    tabBarController.viewControllers = [NSArray arrayWithObjects:messageNC, wanglaiNC,contactNC, meNC, nil];
    tabBarController.tabBar.barStyle = UIBarStyleBlack;
    tabBarController.tabBar.translucent = NO;
    tabBarController.tabBar.selectedImageTintColor = [UIColor cyanColor];

    self.window.rootViewController = tabBarController;
    
  //  XMPPvCardTemp *myvCardTemp = [[self xmppvCardTempModule]
    //                              myvCardTemp];
    
  //  XMPPvCardTemp *myvCardTemp = [xmppvCardTempModule vCardTempForJID:[XMPPJID jidWithString:XMPPmyJID] shouldFetch:YES];
    
  //  NSLog(@"myvcard = %@",myvCardTemp);
    ...
    if (![self connect])
	{
        ...
        loginNC.navigationBar.barStyle = UIBarStyleBlack;
        loginNC.navigationBar.translucent = NO;
		dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, 0.0 * NSEC_PER_SEC);
		dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
			
			[tabBarController presentViewController:loginNC animated:YES completion:NULL];
		});
	}
...
static XMPPAppDelegate *sharedManager;

+ (XMPPAppDelegate *)sharedInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedManager=[[XMPPAppDelegate alloc]init];
...
- (BOOL)connect
{
    ...
    //设置服务器
    [xmppStream setHostName:Wanglai_HostIP];
	password = myPassword;
...
//    }
    ...
    [msg setMessageContent:message.body];
    [msg setMessageFrom:[[[message fromStr] componentsSeparatedByString:@"/"] objectAtIndex:0]];
  
    [msg setMessageTo:[[[message toStr] componentsSeparatedByString:@"/"] objectAtIndex:0]];
    [msg setMessageType:[NSNumber numberWithInt:0]];
    [msg setMessageDate:[NSDate dateWithTimeIntervalSinceNow:1]];
	...
	if ([message isChatMessageWithBody])
	{
		...
		{
            [[NSNotificationCenter defaultCenter]postNotificationName:@"xmppNewMsgNotifaction" object:message ];
...
//    
//    [contactDelegate allRoster:[NSString stringWithFormat:@"%@", name]];
//
//}
